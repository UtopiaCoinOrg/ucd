...
ucjson.MarshalCmd and ucjson.MarshalResponse functions may be used.  They
return the raw bytes ready to be sent across the wire.

Unmarshalling a received Request object is a two step process:
  1) Unmarshal the raw bytes into a ucjson.Request struct instance via
     json.Unmarshal
  2) Use ucjson.ParseParams on the Method and Params fields of the unmarshalled
...
Unmarshalling a received Response object is also a two step process:
  1) Unmarshal the raw bytes into a ucjson.Response struct instance via
     json.Unmarshal
...
The second approach is the ucjson.NewCmd function which takes a method
(command) name and variable arguments.  Since this package registers all of its
types with ucjson, the function will recognize them and includes full checking
...
To facilitate providing consistent help to users of the RPC server, the ucjson
...
In addition, the ucjson.MethodUsageText function may be used to generate
