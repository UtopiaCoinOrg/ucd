...
// Client represents a Utopia RPC client which allows easy access to the
// various RPC methods available on a Utopia RPC server.  Each of the wrapper
...
type (
	...
	rawResponse struct {
		...
		Error  *ucjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled ucjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := ucjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := ucjson.MarshalCmd("1.0", id, cmd)
	if err != nil {
		return newFutureError(err)
	}
