...
func (c *Client) GetBlockAsync(blockHash *chainhash.Hash) FutureGetBlockResult {
	...
	cmd := chainjson.NewGetBlockCmd(hash, ucjson.Bool(false), nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) GetBlockVerboseAsync(blockHash *chainhash.Hash, verboseTx bool) FutureGetBlockVerboseResult {
	...
	cmd := chainjson.NewGetBlockCmd(hash, ucjson.Bool(true), &verboseTx)
	return c.sendCmd(cmd)
}
...
func (c *Client) GetBlockHeaderAsync(hash *chainhash.Hash) FutureGetBlockHeaderResult {
	cmd := chainjson.NewGetBlockHeaderCmd(hash.String(), ucjson.Bool(false))
	return c.sendCmd(cmd)
}
...
func (c *Client) GetBlockHeaderVerboseAsync(hash *chainhash.Hash) FutureGetBlockHeaderVerboseResult {
	cmd := chainjson.NewGetBlockHeaderCmd(hash.String(), ucjson.Bool(true))
	return c.sendCmd(cmd)
}
...
// Receive waits for the response promised by the future and returns the
// current coin supply
func (r FutureGetCoinSupplyResult) Receive() (ucutil.Amount, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return 0, err
	}
...
func (c *Client) GetCoinSupply() (ucutil.Amount, error) {
...
func (c *Client) GetRawMempoolAsync(txType chainjson.GetRawMempoolTxTypeCmd) FutureGetRawMempoolResult {
	cmd := chainjson.NewGetRawMempoolCmd(ucjson.Bool(false),
		ucjson.String(string(txType)))
	return c.sendCmd(cmd)
}
...
func (c *Client) GetRawMempoolVerboseAsync(txType chainjson.GetRawMempoolTxTypeCmd) FutureGetRawMempoolVerboseResult {
	cmd := chainjson.NewGetRawMempoolCmd(ucjson.Bool(true),
		ucjson.String(string(txType)))
	return c.sendCmd(cmd)
}
...
func (r FutureEstimateSmartFeeResult) Receive() (float64, error) {
	...
	// Unmarshal the result as a float64.
	var ucPerKB float64
	err = json.Unmarshal(res, &ucPerKB)
	if err != nil {
		return 0, err
	}
	return ucPerKB, nil
...
// EstimateSmartFee returns an estimation of a transaction fee rate (in uc/KB)
...
//
// As of 2019-01, only the default conservative mode is supported by ucd.
