...
ucd or ucwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by ucd and
ucwallet only uses a single connection that remains open and allows
...
All notifications provided by ucd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by ucd (and
ucwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that ucd intentionally separates the wallet
functionality into a separate process named ucwallet.  This means if you are
connected to the ucd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, ucwallet provides pass through treatment
...
detected by type asserting the error in a *ucjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*ucjson.RPCError); ok {
  		switch jerr.Code {
  		case ucjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - ucdwebsockets
   ...
   block count
 - ucwalletwebsockets
   Connects to a ucwallet RPC server using TLS-secured websockets, registers
