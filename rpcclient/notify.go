...
type NotificationHandlers struct {
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount ucutil.Amount)
	...
	// OnUcdConnected is invoked when a wallet connects or disconnects from
	// ucd.
	//
	// This will only be available when client is connected to a wallet
	// server such as ucwallet.
	OnUcdConnected func(connected bool)

	// OnAccountBalance is invoked with account balance updates.
	//
	// This will only be available when speaking to a wallet server
	// such as ucwallet.
	OnAccountBalance func(account string, balance ucutil.Amount, confirmed bool)
	...
	// This will only be available when client is connected to a wallet
	// server such as ucwallet.
	OnTicketsPurchased func(TxHash *chainhash.Hash, amount ucutil.Amount)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnUcdConnected
	case walletjson.UcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		if c.ntfnHandlers.OnUcdConnected == nil {
			return
		}

		connected, err := parseUcdConnectedNtfnParams(ntfn.Params)
		if err != nil {
			log.Warnf("Received invalid ucd connected "+
				"notification: %v", err)
			return
		}
...
// parseUcdConnectedNtfnParams parses out the connection status of ucd
// and ucwallet from the parameters of a ucdconnected notification.
func parseUcdConnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance ucutil.Amount, confirmed bool, err error) {
...
func parseTicketPurchasedNtfnParams(params []json.RawMessage) (txHash *chainhash.Hash,
	amount ucutil.Amount, err error) {
	...
	return thHash, ucutil.Amount(amt), nil
}
...
//
// NOTE: This is a ucd extension and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []ucutil.Address,
...
//
// NOTE: This is a ucd extension and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []ucutil.Address, outPoints []wire.OutPoint) error {
