...
// blockMsg packages a Utopia block message and the peer it came from together
...
// invMsg packages a Utopia inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a Utopia headers message and the peer it came from
...
// txMsg packages a Utopia tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *ucutil.Tx
	peer *serverPeer
...
type PeerNotifier interface {
	...
	TransactionConfirmed(tx *ucutil.Tx)
}
...
func (b *blockManager) checkBlockForHiddenVotes(block *ucutil.Block) {
	...
	votesFromBlock := make([]*ucutil.Tx, 0,
	...
	var newVotes []*ucutil.Tx
	var oldTickets []*ucutil.Tx
	var oldRevocations []*ucutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.cfg.ChainParams.TicketsPerBlock))
	templateBlock := ucutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*ucutil.Tx, 0,
	...
	updatedTxTreeRegular := make([]*ucutil.Tx, 0,
...
func headerApprovesParent(header *wire.BlockHeader) bool {
	return ucutil.IsFlagSet16(header.VoteBits, ucutil.BlockValid)
}
...
func (b *blockManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTNewTipBlockChecked:
		...
		block, ok := notification.Data.(*ucutil.Block)
		if !ok {
	...
	case blockchain.NTBlockConnected:
		...
		handleConnectedBlockTxns := func(txns []*ucutil.Tx) {
			for _, tx := range txns {
	...
	case blockchain.NTBlockDisconnected:
		...
		handleDisconnectedBlockTxns := func(txns []*ucutil.Tx) {
			for _, tx := range txns {
				_, err := txMemPool.MaybeAcceptTransaction(tx, false, true)
				if err != nil && !isDoubleSpendOrDuplicateError(err) {
					txMemPool.RemoveTransaction(tx, true)
...
// queue.
func (b *blockManager) QueueTx(tx *ucutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *ucutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *ucutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *ucutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*ucutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (ucutil.Amount, error) {
...
// newBlockManager returns a new Utopia block manager.
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		ucdLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		ucdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		ucdLog.Infof("Creating block database in memory.")
	...
	ucdLog.Infof("Loading block database from '%s'", dbPath)
	...
	ucdLog.Info("Block database loaded")
	return db, nil
}
