...
import (
	...
	"github.com/Utopia/ucd/mempool"
	"github.com/Utopia/ucd/rpc/jsonrpc/types"
	"github.com/Utopia/ucd/sampleconfig"
...
const (
	defaultConfigFilename        = "ucd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "ucd.log"
	defaultMaxSameIP             = 5
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = ucutil.AppDataDir("ucd", false)
...
type config struct {
	...
	Listeners            []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 10508, testnet: 11508)"`
	...
	RPCListeners         []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 10509, testnet: 11509)"`
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in UC/kB to be considered a non-zero fee."`
	...
	AltDNSNames          []string      `long:"altdnsnames" description:"Specify additional dns names to use when generating the rpc server certificate" env:"UCD_ALT_DNSNAMES" env-delim:","`
...
// createDefaultConfig copies the file sample-ucd.conf to the given destination path,
...
//
// The above results in ucd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for ucd if specified. Since the home
	...
	if cfg.TestNet {
		...
		ucutil.ActiveNet = chaincfg.TestNet3Params()
	}
	if cfg.SimNet {
		numNets++
		// Also disable dns seeding on the simulation test network.
		activeNetParams = &simNetParams
		ucutil.ActiveNet = chaincfg.SimNetParams()
		cfg.DisableDNSSeed = true
	}
	if cfg.RegNet {
		numNets++
		activeNetParams = &regNetParams
		ucutil.ActiveNet = chaincfg.RegNetParams()
	}
	if numNets > 1 {
	...
	cfg.minRelayTxFee, err = ucutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]ucutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := ucutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			ucdLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		ucdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// ucdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func ucdDial(network, addr string) (net.Conn, error) {
...
// ucdLookup returns the correct DNS lookup function to use depending on the
...
func ucdLookup(host string) ([]net.IP, error) {
