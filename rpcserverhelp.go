...
var helpDescsEnUS = map[string]string{
	...
	"debuglevel--synopsis": "Dynamically changes the debug logging level.\n" +
		...
		"The valid subsystems are AMGR, ADXR, BCDB, BMGR, UCD, CHAN, DISC, PEER, RPCS, SCRP, SRVR, and TXMP.\n" +
	...
	"createrawsstx-amount--desc":  "The destination address as the key and the amount in UC as the value",
	...
	"createrawtransaction-amounts--desc":  "The destination address as the key and the amount in UC as the value",
	...
	"scriptpubkeyresult-addresses": "The Utopia addresses associated with this script",
	"scriptpubkeyresult-commitamt": "The ticket commitment value if the script is for a staking commitment",

	// Vout help.
	"vout-value":        "The amount in UC",
	...
	"decodescriptresult-addresses": "The Utopia addresses associated with this script",
	...
	"getblock-verbosetx":   "Specifies that each transaction is returned as a JSON object and only applies if the verbose flag is true (ucd extension)",
	...
	"getcurrentnet--synopsis": "Get Utopia network the server is running on.",
	...
	"infochainresult-relayfee":        "The minimum relay fee for non-free transactions in UC/KB",
	...
	"infowalletresult-balance":         "The total Utopia balance of the wallet",
	...
	"infowalletresult-paytxfee":        "The transaction fee set in UC/KB",
	"infowalletresult-relayfee":        "The minimum relay fee for non-free transactions in UC/KB",
	...
	"getrawmempoolverboseresult-fee":              "Transaction fee in utopia",
	...
	"gettxoutresult-value":         "The transaction amount in UC",
	...
	"searchrawtransactions-address":     "The Utopia address to search for",
	...
	"sendrawtransaction-allowhighfees": "Whether or not to allow insanely high fees (ucd does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown ucd.",
	"stop--result0":  "The string 'ucd stopping.'",
	...
	"validateaddresschainresult-address": "The Utopia address (only when isvalid is true)",

	// ValidateAddressCmd help.
	"validateaddress--synopsis": "Verify an address is valid.",
	"validateaddress-address":   "Utopia address to validate",
	...
	"verifymessage-address":   "The Utopia address to use for the signature",
	...
	// -------- Utopia-specific help --------

	// EstimateFee help.
	"estimatefee--synopsis": "Returns the estimated fee in uc/kb.",
	"estimatefee-numblocks": "(unused)",
	"estimatefee--result0":  "Estimated fee.",

	// EstimateSmartFee help.
	"estimatesmartfee--synopsis":     "Returns the estimated fee using the historical fee data in uc/kb.",
	...
	"estimatesmartfee--result0":      "Estimated fee rate (in UC/KB).",
	...
	// TicketFeeInfo help.
	"ticketfeeinfo--synopsis":            "Get various information about ticket fees from the mempool, blocks, and difficulty windows (units: UC/kB)",
	...
	"ticketfeeinforesult-feeinfomempool": "Ticket fee information for all tickets in the mempool (units: UC/kB)",
	"ticketfeeinforesult-feeinfoblocks":  "Ticket fee information for a given list of blocks descending from the chain tip (units: UC/kB)",
	"ticketfeeinforesult-feeinfowindows": "Ticket fee information for a window period where the stake difficulty was the same (units: UC/kB)",
...
func (c *helpCacher) rpcMethodHelp(method types.Method) (string, error) {
	...
	help, err := ucjson.GenerateHelp(method, helpDescsEnUS, resultTypes...)
	if err != nil {
		return "", err
	}
...
func (c *helpCacher) rpcUsage(includeWebsockets bool) (string, error) {
	...
	for k := range rpcHandlers {
		usage, err := ucjson.MethodUsageText(k)
		if err != nil {
			return "", err
		}
