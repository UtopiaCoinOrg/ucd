...
// Copyright (c) 2017-2019 The Utopia developers
...
type fakeChain struct {
	...
	blocks         map[chainhash.Hash]*ucutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *ucutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*ucutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *ucutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *ucutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
func (s *fakeChain) AddFakeUtxoMedianTime(tx *ucutil.Tx, txOutIdx uint32, medianTime time.Time) {
	s.Lock()
...
// transactions.
func txOutToSpendableOut(tx *ucutil.Tx, outputNum uint32, tree int8) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum, Tree: tree},
		amount:   ucutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// passed in.
func (p *poolHarness) GetScript(addr ucutil.Address) ([]byte, error) {
...
func (p *poolHarness) GetKey(addr ucutil.Address) (chainec.PrivateKey, bool, error) {
	return p.signKey, true, nil
}

// AddFakeUTXO creates a fake mined uxto for the provided transaction.
func (p *poolHarness) AddFakeUTXO(tx *ucutil.Tx, blockHeight int64) {
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*ucutil.Tx, error) {
...
// testing.
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32, mungers ...func(*wire.MsgTx)) (*ucutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput ucutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*ucutil.Tx, error) {
	txChain := make([]*ucutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, ucutil.NewTx(tx))
...
// output.
func (p *poolHarness) CreateTx(out spendableOutput) (*ucutil.Tx, error) {
...
func (p *poolHarness) CreateTicketPurchase(sourceTx *ucutil.Tx, cost int64) (*ucutil.Tx, error) {
	ticketfee := ucutil.Amount(singleInputTicketSize)
	ticketPrice := ucutil.Amount(cost)
	...
	change := ucutil.Amount(sourceTx.MsgTx().TxOut[0].Value) -
...
// the opportunity to modify the transaction which is especially useful for
// testing.
func (p *poolHarness) CreateVote(ticket *ucutil.Tx, mungers ...func(*wire.MsgTx)) (*ucutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, vote, inputToSign,
		...
		p, vote.TxIn[inputToSign].SignatureScript, ucec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return ucutil.NewTx(vote), nil
}

// CreateRevocation creates a revocation using the provided ticket.
func (p *poolHarness) CreateRevocation(ticket *ucutil.Tx) (*ucutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, revocation, inputToSign,
		redeemTicketScript, txscript.SigHashAll, p,
		p, revocation.TxIn[inputToSign].SignatureScript, ucec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return ucutil.NewTx(revocation), nil
}
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := ucutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*ucutil.Block),
...
func testPoolMembership(tc *testContext, tx *ucutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
...
func TestExpirationPruning(t *testing.T) {
	...
	expiringTxns := make([]*ucutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   ucutil.Amount(5000000000),
...
func TestSequenceLockAcceptance(t *testing.T) {
	...
	for _, acceptSeqLocks := range []bool{false, true} {
		...
		for i, test := range tests {
			...
			inputTx := ucutil.NewTx(inputMsgTx)
