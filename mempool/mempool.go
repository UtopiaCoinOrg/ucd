...
const (
	...
	// UC/kB, this results in a maximum allowed high fee of 1 UC/kB.
...
type Config struct {
	...
	FetchUtxoView func(*ucutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*ucutil.Block, error)
	...
	// utxo view.
	CalcSequenceLock func(*ucutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
	...
	// vote in the mempool.
	OnVoteReceived func(voteTx *ucutil.Tx)
}
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in UC/kB to be
	// considered a non-zero fee.
	MinRelayTxFee ucutil.Amount
...
func (mp *TxPool) insertVote(ssgen *ucutil.Tx) {
	...
	vote := ucutil.IsFlagSet16(voteBits, ucutil.BlockValid)
...
// TODO Pruning of the votes map UTOPIA
...
func (mp *TxPool) removeOrphan(tx *ucutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveOrphan(tx *ucutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *ucutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*ucutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *ucutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *ucutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *ucutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *ucutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *ucutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *ucutil.Tx, txType stake.TxType) error {
...
func (mp *TxPool) checkVoteDoubleSpend(vote *ucutil.Tx) error {
	voteTx := vote.MsgTx()
	...
	for _, existingVote := range mp.votes[hashVotedOn] {
		if existingVote.TicketHash == ticketSpent {
			...
			//
			// TODO(utopia): Ideally the votes map and mempool would be kept in
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *ucutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*ucutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
// It should also set the ucutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *ucutil.Tx, isNew, rateLimit, allowHighFees, rejectDupOrphans bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *ucutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *ucutil.Tx) []*ucutil.Tx {
	var acceptedTxns []*ucutil.Tx

	// Start with processing at least the passed transaction.
	processList := []*ucutil.Tx{acceptedTx}
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *ucutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*ucutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*ucutil.Tx, len(newTxs)+1)
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*ucutil.Tx),
