...
const (
	...
	DefaultMinRelayTxFee = ucutil.Amount(1e4)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee ucutil.Amount) int64 {
	...
	if minFee < 0 || minFee > ucutil.MaxAmount {
		minFee = ucutil.MaxAmount
	}
...
func checkInputsStandard(tx *ucutil.Tx, txType stake.TxType, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee ucutil.Amount) bool {
...
func checkTransactionStandard(tx *ucutil.Tx, txType stake.TxType, height int64,
	medianTime time.Time, minRelayTxFee ucutil.Amount,
