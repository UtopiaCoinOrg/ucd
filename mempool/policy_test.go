...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee ucutil.Amount // minimum relay transaction fee.
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: ucutil.MaxAmount, Version: 0, PkScript: pkScript},
			ucutil.MaxAmount,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := ucutil.NewAddressPubKeyHash(addrHash[:],
		&chaincfg.RegNetParams, ucec.STEcdsaSecp256k1)
	if err != nil {
