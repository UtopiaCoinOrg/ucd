...
// arguments returns an array of arguments that be used to launch the ucd
// process.
...
// command returns the exec.Cmd which will be used to start the ucd process.
...
// to the ucd process that is launched via Start().
...
// as the base for the log and data directories for ucd.
...
// start creates a new ucd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "ucd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running ucd process process, and waits until it exits
...
// shutdown terminates the running ucd process, and cleans up all
