...
import (
	...
	"github.com/Utopia/ucd/ucjson/v2"
...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt ucutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(ucutil.Amount(5 * ucutil.AtomsPerCoin))
	...
	txid = genSpend(ucutil.Amount(5000 * ucutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(ucjson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(ucjson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := ucutil.Amount(5 * 300 * ucutil.AtomsPerCoin)
	...
	// chain.
	expectedBalance = ucutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := ucutil.Amount(50 * ucutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := ucutil.Amount(numMatureOutputs * 300 * ucutil.AtomsPerCoin)
