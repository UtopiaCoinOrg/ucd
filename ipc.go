...
// processes to communicate with and and manage the lifetime of a ucd child
...
// message, shutdown begins.  This prevents ucd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			ucdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child ucd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	ucdLog.Errorf("Failed to write to pipe: %v", err)
}
