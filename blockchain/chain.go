...
// BlockChain provides functions for working with the Utopia block chain.
...
type BlockChain struct {
	...
	mainchainBlockCache     map[chainhash.Hash]*ucutil.Block
...
// StakeVersions is a condensed form of a ucutil.Block that is used to prevent
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *ucutil.Block) {
...
func (b *BlockChain) fetchMainChainBlockByNode(node *blockNode) (*ucutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchBlockByNode(node *blockNode) (*ucutil.Block, error) {
...
func (b *BlockChain) pushMainChainBlockCache(block *ucutil.Block) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block, parent *ucutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
	...
	blockAndParent := []*ucutil.Block{block, parent}
...
func (b *BlockChain) dropMainChainBlockCache(block *ucutil.Block) {
	curHash := block.Hash()
	b.mainchainBlockCacheLock.Lock()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block, parent *ucutil.Block, view *UtxoViewpoint) error {
...
func countSpentRegularOutputs(block *ucutil.Block) int {
...
func countSpentStakeOutputs(block *ucutil.Block) int {
	var numSpent int
...
func countSpentOutputs(block *ucutil.Block) int {
...
func (b *BlockChain) reorganizeChainInternal(targetTip *blockNode) error {
	...
	var nextBlockToDetach *ucutil.Block
	...
	var prevBlockAttached *ucutil.Block
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block, parent *ucutil.Block, flags BehaviorFlags) (int64, error) {
...
func (b *BlockChain) isCurrent() bool {
	...
	// otherwise.
	minus24Hours := b.timeSource.AdjustedTime().Add(-24 * 7 * time.Hour).Unix()
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*ucutil.Block, error) {
	node := b.index.LookupNode(hash)
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(height int64) (*ucutil.Block, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *ucutil.Block, *ucutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *ucutil.Block, *ucutil.Block, *UtxoViewpoint) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		mainchainBlockCache:           make(map[chainhash.Hash]*ucutil.Block),
