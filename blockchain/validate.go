...
func voteBitsApproveParent(voteBits uint16) bool {
	return ucutil.IsFlagSet16(voteBits, ucutil.BlockValid)
}
...
func IsCoinBase(tx *ucutil.Tx) bool {
...
// level util transaction as opposed to a raw wire transaction.
func IsExpired(tx *ucutil.Tx, blockHeight int64) bool {
...
func IsFinalizedTransaction(tx *ucutil.Tx, blockHeight int64, blockTime time.Time) bool {
...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// Utopia is a quantity of atoms as defined by the AtomsPerCoin constant.
	//
...
func checkProofOfStake(block *ucutil.Block, posLimit int64) error {
...
func checkBlockSanity(block *ucutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
	msgBlock := block.MsgBlock()
...
// sane before continuing with block processing.  These checks are context
// free.
func CheckBlockSanity(block *ucutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
func (b *BlockChain) checkBlockPositional(block *ucutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
// impossible.
func checkCoinbaseUniqueHeight(blockHeight int64, block *ucutil.Block) error {
...
func (b *BlockChain) checkBlockContext(block *ucutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
// NOTE: The caller MUST have already determined that the provided transaction
// is a vote.
func checkVoteInputs(subsidyCache *SubsidyCache, tx *ucutil.Tx, txHeight int64, view *UtxoViewpoint, params *chaincfg.Params) error {
...
// NOTE: The caller MUST have already determined that the provided transaction
// is a revocation.
func checkRevocationInputs(tx *ucutil.Tx, txHeight int64, view *UtxoViewpoint, params *chaincfg.Params) error {
	ticketMaturity := int64(params.TicketMaturity)
	revokeHash := tx.Hash()
	msgTx := tx.MsgTx()
...
// owner of the Utopia and therefore allowed to spend them.  As it checks the
// inputs, it also calculates the total fees for the transaction and returns
// that value.
//
// NOTE: The transaction MUST have already been sanity checked with the
// CheckTransactionSanity function prior to calling this function.
func CheckTransactionInputs(subsidyCache *SubsidyCache, tx *ucutil.Tx, txHeight int64, view *UtxoViewpoint, checkFraudProof bool, chainParams *chaincfg.Params) (int64, error) {
	...
	// -------------------------------------------------------------------
	// Utopia stake transaction testing.
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// Utopia general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	...
	for idx, txIn := range msgTx.TxIn {
		...
		// Utopia is a quantity of atoms as defined by the AtomPerCoin
		// constant.
		...
		if originTxAtom > ucutil.MaxAmount {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				originTxAtom, ucutil.MaxAmount)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
...
// txscript.
func CountSigOps(tx *ucutil.Tx, isCoinBaseTx bool, isSSGen bool) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *ucutil.Tx, isCoinBaseTx bool, isStakeBaseTx bool, view *UtxoViewpoint) (int, error) {
...
func (b *BlockChain) createLegacySeqLockView(block, parent *ucutil.Block, view *UtxoViewpoint) (*UtxoViewpoint, error) {
...
func checkNumSigOps(tx *ucutil.Tx, view *UtxoViewpoint, index int, txTree bool, cumulativeSigOps int) (int, error) {
	msgTx := tx.MsgTx()
	isSSGen := stake.IsSSGen(msgTx)
...
func checkStakeBaseAmounts(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*ucutil.Tx, view *UtxoViewpoint) error {
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeBaseAmounts(txs []*ucutil.Tx, view *UtxoViewpoint) (int64, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeTreeFees(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*ucutil.Tx, view *UtxoViewpoint) (ucutil.Amount, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
	...
	return ucutil.Amount(totalInputs - totalOutputs), nil
}
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block, parent *ucutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CheckConnectBlockTemplate(block *ucutil.Block) error {
	b.chainLock.Lock()
