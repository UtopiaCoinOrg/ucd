...
import (
	...
	"github.com/UtopiaCoinOrg/ucd/blockchain/internal/dbnamespace"
...
func serializeSizeForMinimalOutputs(tx *ucutil.Tx) int {
...
func putTxToMinimalOutputs(target []byte, tx *ucutil.Tx) int {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *ucutil.Block) error {
...
// The amount is never encoded into spent transaction outputs in Utopia
...
func decodeSpentTxOut(serialized []byte, stxo *spentTxOut, amount int64, height uint32, index uint32) (int, error) {
	...
	// since in Utopia we only need pkScript at most due to fraud proofs
...
// txouts.
func dbFetchSpendJournalEntry(dbTx database.Tx, block *ucutil.Block) ([]spentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := ucutil.NewBlock(b.chainParams.GenesisBlock)
	...
	err := b.db.Update(func(dbTx database.Tx) error {
		...
		_, err = stake.InitDatabaseState(dbTx, b.chainParams, &b.chainParams.GenesisHash)
		if err != nil {
			return err
		}
...
// block for the provided node, deserialize it, and return a ucutil.Block.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*ucutil.Block, error) {
	...
	block, err := ucutil.NewBlockFromBytes(blockBytes)
	if err != nil {
		return nil, err
	}
