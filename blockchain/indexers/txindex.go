...
func dbAddTxIndexEntries(dbTx database.Tx, block *ucutil.Block, blockID uint32) error {
	...
	addEntries := func(txns []*ucutil.Tx, txLocs []wire.TxLoc, blockID uint32) error {
		offset := 0
...
// block (if they were valid) and every stake transaction in the passed block.
func dbRemoveTxIndexEntries(dbTx database.Tx, block *ucutil.Block) error {
	removeEntries := func(txns []*ucutil.Tx) error {
		for _, tx := range txns {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) ConnectBlock(dbTx database.Tx, block, parent *ucutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) DisconnectBlock(dbTx database.Tx, block, parent *ucutil.Block, view *blockchain.UtxoViewpoint) error {
	// NOTE: The fact that the block can disapprove the regular tree of the
