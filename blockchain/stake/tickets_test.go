...
// throughout the tests.  They match the Utopia regression test network params
...
func ticketsInBlock(bl *ucutil.Block) []chainhash.Hash {
...
func ticketsSpentInBlock(bl *ucutil.Block) []chainhash.Hash {
...
func revokedTicketsInBlock(bl *ucutil.Block) []chainhash.Hash {
...
func TestTicketDBLongChain(t *testing.T) {
	...
	testBlockchain := make(map[int64]*ucutil.Block, len(testBlockchainBytes))
	for k, v := range testBlockchainBytes {
		bl, err := ucutil.NewBlockFromBytes(v)
		if err != nil {
