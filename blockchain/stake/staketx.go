...
// commitment pkScript.
func AddrFromSStxPkScrCommitment(pkScript []byte, params ucutil.AddressParams) (ucutil.Address, error) {
	...
	if isP2SH {
		return ucutil.NewAddressScriptHashFromHash(hashBytes, params)
	}
	return ucutil.NewAddressPubKeyHash(hashBytes, params,
		ucec.STEcdsaSecp256k1)
}

// AmountFromSStxPkScrCommitment extracts a commitment amount from a
// ticket commitment pkScript.
func AmountFromSStxPkScrCommitment(pkScript []byte) (ucutil.Amount, error) {
	if len(pkScript) < SStxPKHMinOutSize {
	...
	return ucutil.Amount(binary.LittleEndian.Uint64(amtEncoded)), nil
}
