...
//
// This function is safe for concurrent access.
func (b *BlockChain) TicketsWithAddress(address ucutil.Address) ([]chainhash.Hash, error) {
	b.chainLock.RLock()
	sn := b.bestChain.Tip().stakeNode
	b.chainLock.RUnlock()
	...
	err := b.db.View(func(dbTx database.Tx) error {
		for _, hash := range tickets {
			...
			if addrs[0].String() == address.Address() {
...
func (b *BlockChain) TicketPoolValue() (ucutil.Amount, error) {
	b.chainLock.RLock()
	sn := b.bestChain.Tip().stakeNode
	b.chainLock.RUnlock()
