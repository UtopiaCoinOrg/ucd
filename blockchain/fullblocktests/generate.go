...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := ucutil.Hash160(redeemScript)
...
// subsidy.
func additionalCoinbasePoW(amount ucutil.Amount) func(*wire.MsgBlock) {
...
func additionalCoinbaseDev(amount ucutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		// Increase the first proof-of-work coinbase subsidy by the
		// provided amount.
...
func additionalSpendFee(fee ucutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	g.NextBlock("bbadtaxscript", outs[5], ticketOuts[5],
		func(b *wire.MsgBlock) {
			...
			p2shTaxAddr := addrs[0].(*ucutil.AddressScriptHash)
			p2pkhTaxAddr, err := ucutil.NewAddressPubKeyHash(
	...
	g.NextBlock("bv15", outs[9], ticketOuts[9], func(b *wire.MsgBlock) {
		ticketFee := ucutil.Amount(2)
		ticketPrice := ucutil.Amount(g.CalcNextReqStakeDifficulty(g.Tip()))
		ticketPrice--
	...
	// vector is not possible in Utopia since the block size is included in
	...
	// Utopia enforces the coinbase includes the block height to which it
	...
	g.NextBlock("bcb9", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = ucutil.MaxAmount + 1
	})
	rejected(blockchain.ErrBadTxOutValue)
	...
	g.NextBlock("bcb10", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = ucutil.MaxAmount
