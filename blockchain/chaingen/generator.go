...
func (s *SpendableOut) Amount() ucutil.Amount {
	return s.amount
}
...
func makeSpendableOutForTxInternal(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32, tree int8) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      ucutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func MakeGenerator(params *chaincfg.Params) (Generator, error) {
	...
	p2shOpTrueAddr, err := ucutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
// a single OP_TRUE.
func (g *Generator) P2shOpTrueAddr() ucutil.Address {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) ucutil.Amount {
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy ucutil.Amount, blockHeight uint32, numVotes uint16) ucutil.Amount {
	powProportion := ucutil.Amount(g.params.WorkRewardProportion)
	totalProportions := ucutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := ucutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * ucutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) ucutil.Amount {
	if int64(heightVotedOn+1) < g.params.StakeValidationHeight {
		return 0
	}

	fullSubsidy := g.calcFullSubsidy(heightVotedOn)
	posProportion := ucutil.Amount(g.params.StakeRewardProportion)
	totalProportions := ucutil.Amount(g.params.TotalSubsidyProportions())
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy ucutil.Amount, blockHeight uint32, numVotes uint16) ucutil.Amount {
	devProportion := ucutil.Amount(g.params.BlockTaxProportion)
	totalProportions := ucutil.Amount(g.params.TotalSubsidyProportions())
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := ucutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * ucutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy ucutil.Amount) {
...
func PurchaseCommitmentScript(addr ucutil.Address, amount, voteFeeLimit, revocationFeeLimit ucutil.Amount) []byte {
...
func (g *Generator) CreateTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee ucutil.Amount) *wire.MsgTx {
...
func (g *Generator) CreateVoteTx(voteBlock *wire.MsgBlock, ticketTx *wire.MsgTx, ticketBlockHeight, ticketBlockIndex uint32) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / ucutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := ucutil.Amount(ticketTx.TxOut[0].Value)
...
// children nodes.  A diagram depicting how this works for Utopia transactions
...
// will be nil.
func buildMerkleTreeStore(transactions []*ucutil.Tx) []*chainhash.Hash {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*ucutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, ucutil.NewTx(tx))
	}
...
//
// This compact form is only used in Utopia to encode unsigned 256-bit numbers
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee ucutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee ucutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := ucutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = ucutil.Amount(2)
	...
	var stakeTreeFees ucutil.Amount
	for _, tx := range stakeTxns {
		...
		var inputSum, outputSum ucutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += ucutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += ucutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount ucutil.Amount, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	var totalSubsidy ucutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		payoutAddr, err := ucutil.DecodeAddress(payout.Address)
		if err != nil {
			panic(err)
		}
		...
		totalSubsidy += ucutil.Amount(payout.Amount)
	}
