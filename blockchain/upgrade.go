...
// block.
func ticketsVotedInBlock(bl *ucutil.Block) []chainhash.Hash {
...
// block.
func ticketsRevokedInBlock(bl *ucutil.Block) []chainhash.Hash {
	var tickets []chainhash.Hash
	for _, stx := range bl.MsgBlock().STransactions {
...
func upgradeToVersion2(db database.DB, chainParams *chaincfg.Params, dbInfo *databaseInfo) error {
	...
	dbFetchBlockByHeight := func(dbTx database.Tx, height int64) (*ucutil.Block, error) {
	...
	err := db.Update(func(dbTx database.Tx) error {
		...
		bestStakeNode, errLocal := stake.InitDatabaseState(dbTx, chainParams, &chainParams.GenesisHash)
		if errLocal != nil {
			return errLocal
		}
...
func upgradeToVersion5(db database.DB, chainParams *chaincfg.Params, dbInfo *databaseInfo, interrupt <-chan struct{}) error {
	...
	err = db.Update(func(dbTx database.Tx) error {
		...
		if err := stake.ResetDatabase(dbTx, chainParams, &chainParams.GenesisHash); err != nil {
			return err
		}
