...
const (
	...
	unusableFlags = ucjson.UFWebsocketOnly | ucjson.UFNotification
)

var (
	ucdHomeDir            = ucutil.AppDataDir("ucd", false)
	ucctlHomeDir          = ucutil.AppDataDir("ucctl", false)
	ucwalletHomeDir       = ucutil.AppDataDir("ucwallet", false)
	defaultConfigFile      = filepath.Join(ucctlHomeDir, "ucctl.conf")
	defaultRPCServer       = "localhost"
	defaultWalletRPCServer = "localhost"
	defaultRPCCertFile     = filepath.Join(ucdHomeDir, "rpc.cert")
	defaultWalletCertFile  = filepath.Join(ucwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	for i := range categories {
		...
		methods := ucjson.RegisteredMethods(method)
		for _, methodStr := range methods {
			switch method.(type) {
			case ucdtypes.Method:
				method = ucdtypes.Method(methodStr)
			case wallettypes.Method:
				method = wallettypes.Method(methodStr)
			}

			flags, err := ucjson.MethodUsageFlags(method)
			if err != nil {
			...
			usage, err := ucjson.MethodUsageText(method)
			if err != nil {
				// This should never happen since the method was just
				// returned from the package, but be safe.
				continue
...
// config defines the configuration options for ucctl.
//
...
// For this it tries to read the ucd config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing ucd conf file at the default
	// path to extract the details from.
	ucdConfigPath := filepath.Join(ucdHomeDir, "ucd.conf")
	if !fileExists(ucdConfigPath) {
		return nil
	}

	// Read ucd.conf from its default path
	ucdConfigFile, err := os.Open(ucdConfigPath)
	if err != nil {
		return err
	}
	defer ucdConfigFile.Close()
	content, err := ioutil.ReadAll(ucdConfigFile)
	if err != nil {
		return err
	}
