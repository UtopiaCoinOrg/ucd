...
func main() {
	...
	methodStr := args[0]
	var method interface{} = ucdtypes.Method(methodStr)
	usageFlags, err := ucjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := ucjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// ucjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(ucjson.Error); ok {
		...
		// The error is not a ucjson.Error and this really should not
	...
	marshalledJSON, err := ucjson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
