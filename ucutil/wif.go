...
func NewWIF(privKey chainec.PrivateKey, net [2]byte, ecType ucec.SignatureType) *WIF {
...
func DecodeWIF(wif string, net [2]byte) (*WIF, error) {
	...
	var ecType ucec.SignatureType
	switch ucec.SignatureType(decoded[2]) {
...
func (w *WIF) DSA() ucec.SignatureType {
	return w.ecType
}
