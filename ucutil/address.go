...
func encodePKAddress(serializedPK []byte, netID [2]byte, algo ucec.SignatureType) string {
	pubKeyBytes := []byte{0x00}

	switch algo {
	case ucec.STEcdsaSecp256k1:
		pubKeyBytes[0] = byte(ucec.STEcdsaSecp256k1)
	case ucec.STEd25519:
		pubKeyBytes[0] = byte(ucec.STEd25519)
	case ucec.STSchnorrSecp256k1:
		pubKeyBytes[0] = byte(ucec.STSchnorrSecp256k1)
	}

	// Pubkeys are encoded as [0] = type/ybit, [1:33] = serialized pubkey
	compressed := serializedPK
	if algo == ucec.STEcdsaSecp256k1 || algo == ucec.STSchnorrSecp256k1 {
...
func NewAddressPubKey(decoded []byte, net AddressParams) (Address, error) {
	if len(decoded) == 33 {
		...
		switch ucec.SignatureType(suite) {
		case ucec.STEcdsaSecp256k1:
...
func DecodeAddress(addr string) (Address, error) {
	...
	case net.AddrIDPubKeyHashECDSAV0():
		return NewAddressPubKeyHash(decoded, net, ucec.STEcdsaSecp256k1)

	case net.AddrIDPubKeyHashEd25519V0():
		return NewAddressPubKeyHash(decoded, net, ucec.STEd25519)

	case net.AddrIDPubKeyHashSchnorrV0():
		return NewAddressPubKeyHash(decoded, net, ucec.STSchnorrSecp256k1)
...
type AddressPubKeyHash struct {
	...
	netID [2]byte
	dsa   ucec.SignatureType
}

// NewAddressPubKeyHash returns a new AddressPubKeyHash.  pkHash must
// be 20 bytes.
func NewAddressPubKeyHash(pkHash []byte, net AddressParams, algo ucec.SignatureType) (*AddressPubKeyHash, error) {
...
// hash.
func (a *AddressPubKeyHash) DSA() ucec.SignatureType {
	return a.dsa
}
...
func NewAddressSecpPubKey(serializedPubKey []byte, net AddressParams) (*AddressSecpPubKey, error) {
	...
	// from ucec, but do it here to avoid API churn.  We already know the
...
// differs with the format.  At the time of this writing, most Utopia addresses
...
func (a *AddressSecpPubKey) String() string {
	return encodePKAddress(a.serialize(), a.pubKeyID, ucec.STEcdsaSecp256k1)
}
...
// address.
func (a *AddressSecpPubKey) DSA() ucec.SignatureType {
	return ucec.STEcdsaSecp256k1
}
...
func (a *AddressEdwardsPubKey) String() string {
	return encodePKAddress(a.serialize(), a.pubKeyID, ucec.STEd25519)
}

// AddressPubKeyHash returns the pay-to-pubkey address converted to a
// pay-to-pubkey-hash address.
func (a *AddressEdwardsPubKey) AddressPubKeyHash() *AddressPubKeyHash {
	addr := &AddressPubKeyHash{netID: a.pubKeyHashID, dsa: ucec.STEd25519}
	copy(addr.hash[:], Hash160(a.serialize()))
	return addr
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressEdwardsPubKey) DSA() ucec.SignatureType {
	return ucec.STEd25519
}
...
func (a *AddressSecSchnorrPubKey) String() string {
	return encodePKAddress(a.serialize(), a.pubKeyID, ucec.STSchnorrSecp256k1)
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressSecSchnorrPubKey) DSA() ucec.SignatureType {
	return ucec.STSchnorrSecp256k1
}
