...
// STx returns a wrapped transaction (ucutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (ucutil.Tx) for all
...
// instead provides easy access to wrapped versions (ucutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (ucutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (ucwire.MsgTx) in the underlying wire.MsgBlock, however it
