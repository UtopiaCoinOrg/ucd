...
func (stats *Estimator) EstimateFee(targetConfs int32) (ucutil.Amount, error) {
	stats.lock.RLock()
...
func (stats *Estimator) AddMemPoolTransaction(txHash *chainhash.Hash, fee, size int64, txType stake.TxType) {
	...
	// minimum at values less than 0.001 UC/KB. This is needed because due to
...
func (stats *Estimator) ProcessBlock(block *ucutil.Block) error {
	stats.lock.Lock()
	defer stats.lock.Unlock()
