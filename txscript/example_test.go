...
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a ucutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	mainNetParams := chaincfg.MainNetParams()
	addressStr := "DsSej1qR3Fyc8kV176DCh9n9cY9nqf9Quxk"
	address, err := ucutil.DecodeAddress(addressStr, mainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	pubKeyHash := ucutil.Hash160(pubKey.SerializeCompressed())
	mainNetParams := chaincfg.MainNetParams()
	addr, err := ucutil.NewAddressPubKeyHash(pubKeyHash, mainNetParams,
	...
	// contains a single output that pays to address in the amount of 1 UC.
	...
	lookupKey := func(a ucutil.Address) (chainec.PrivateKey, bool, error) {
	...
	sigScript, err := txscript.SignTxOutput(mainNetParams, redeemTx, 0,
		...
		txscript.KeyClosure(lookupKey), nil, nil, ucec.STEcdsaSecp256k1)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleScriptTokenizer() {
	...
	// some other source.
	hash160 := ucutil.Hash160([]byte("example"))
