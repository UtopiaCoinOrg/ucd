...
// nil otherwise.
func extractPubKeyAltDetails(script []byte) ([]byte, ucec.SignatureType) {
	...
	if len(script) == 35 && script[0] == OP_DATA_32 &&
		isSmallInt(script[33]) && asSmallInt(script[33]) == ucec.STEd25519 {

		return script[1:33], ucec.STEd25519
	}

	if len(script) == 36 && script[0] == OP_DATA_33 &&
		isSmallInt(script[34]) &&
		asSmallInt(script[34]) == ucec.STSchnorrSecp256k1 &&
		isStrictPubKeyEncoding(script[1:34]) {

		return script[1:34], ucec.STSchnorrSecp256k1
	}
...
func isStandardAltSignatureType(op byte) bool {
	...
	return sigType == ucec.STEd25519 || sigType == ucec.STSchnorrSecp256k1
}
...
// will return nil otherwise.
func extractPubKeyHashAltDetails(script []byte) ([]byte, ucec.SignatureType) {
	...
	if len(script) == 26 &&
		...
		return script[3:23], ucec.SignatureType(asSmallInt(script[24]))
	}
...
func payToPubKeyHashEdwardsScript(pubKeyHash []byte) ([]byte, error) {
	edwardsData := []byte{byte(ucec.STEd25519)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
func payToPubKeyHashSchnorrScript(pubKeyHash []byte) ([]byte, error) {
	schnorrData := []byte{byte(ucec.STSchnorrSecp256k1)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
// valid SStxs.
func PayToSStx(addr ucutil.Address) ([]byte, error) {
...
// valid SStxs.
func PayToSStxChange(addr ucutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSGen txs.
func PayToSSGen(addr ucutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSRtx.
func PayToSSRtx(addr ucutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// an SStx.
func GenerateSStxAddrPush(addr ucutil.Address, amount ucutil.Amount, limits uint16) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// specified address.
func PayToAddrScript(addr ucutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *ucutil.AddressPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
	...
	case *ucutil.AddressEdwardsPubKey:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
	...
	case *ucutil.AddressSecSchnorrPubKey:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*ucutil.AddressSecpPubKey, nrequired int) ([]byte, error) {
...
func pubKeyHashToAddrs(hash []byte, params ucutil.AddressParams) []ucutil.Address {
	// Skip the pubkey hash if it's invalid for some reason.
	var addrs []ucutil.Address
	addr, err := ucutil.NewAddressPubKeyHash(hash, params,
...
func scriptHashToAddrs(hash []byte, params ucutil.AddressParams) []ucutil.Address {
	// Skip the hash if it's invalid for some reason.
	var addrs []ucutil.Address
	addr, err := ucutil.NewAddressScriptHashFromHash(hash, params)
	if err == nil {
		addrs = append(addrs, addr)
	}
	return addrs
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams ucutil.AddressParams) (ScriptClass, []ucutil.Address, int, error) {
	if version != 0 {
	...
	if data, sigType := extractPubKeyHashAltDetails(pkScript); data != nil {
		var addrs []ucutil.Address
		addr, err := ucutil.NewAddressPubKeyHash(data, chainParams, sigType)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if data := extractPubKey(pkScript); data != nil {
		...
		if err == nil {
			addr, err := ucutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	if pk, sigType := extractPubKeyAltDetails(pkScript); pk != nil {
		...
		switch sigType {
		case ucec.STEd25519:
			addr, err := ucutil.NewAddressEdwardsPubKey(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}

		case ucec.STSchnorrSecp256k1:
			addr, err := ucutil.NewAddressSecSchnorrPubKey(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	if details.valid {
		...
		addrs := make([]ucutil.Address, 0, details.numPubKeys)
		for i := 0; i < details.numPubKeys; i++ {
			pubkey, err := secp256k1.ParsePubKey(details.pubKeys[i])
			if err == nil {
				addr, err := ucutil.NewAddressSecpPubKeyCompressed(pubkey,
					chainParams)
				if err == nil {
					addrs = append(addrs, addr)
				}
...
// other script versions.
func ExtractPkScriptAltSigType(pkScript []byte) (ucec.SignatureType, error) {
	if pk, sigType := extractPubKeyAltDetails(pkScript); pk != nil {
		return sigType, nil
	}
...
//
// NOTE: Atomic swaps are not considered standard script types by the ucd
