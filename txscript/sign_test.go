...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr ucutil.Address) (chainec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr ucutil.Address) (
			[]byte, error) {
			return nil, errors.New("nope 3")
		})
	}
	return ScriptClosure(func(addr ucutil.Address) ([]byte,
		error) {
...
func signAndCheck(msg string, tx *wire.MsgTx, idx int, pkScript []byte,
	...
	suite ucec.SignatureType) error {
...
func TestSignTxOutput(t *testing.T) {
	...
	signatureSuites := []ucec.SignatureType{
		ucec.STEcdsaSecp256k1,
		ucec.STEd25519,
		ucec.STSchnorrSecp256k1,
	}
	tx := &wire.MsgTx{
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := ucutil.NewAddressPubKeyHash(
					ucutil.Hash160(pkBytes), testingParams,
					suite)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				// for more details
				address, err := ucutil.NewAddressSecpPubKeyCompressed(pk,
					testingParams)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
		...
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				case ucec.STEd25519:
					...
					key, pk = chainec.Edwards.PrivKeyFromBytes(keyDB)
					pkBytes = pk.SerializeUncompressed()
					address, err = ucutil.NewAddressEdwardsPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
				...
				case ucec.STSchnorrSecp256k1:
					...
					key, pk = chainec.SecSchnorr.PrivKeyFromBytes(keyDB)
					pkBytes = pk.Serialize()
					address, err = ucutil.NewAddressSecSchnorrPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := ucutil.NewAddressPubKeyHash(
					ucutil.Hash160(pkBytes), testingParams, suite)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
					break
				...
				scriptAddr, err := ucutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address1, err := ucutil.NewAddressSecpPubKeyCompressed(pk1,
				testingParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			address2, err := ucutil.NewAddressSecpPubKeyCompressed(pk2,
				testingParams)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*ucutil.AddressSecpPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
		...
		for i := range tx.TxIn {
			...
			sigScript, err = SignTxOutput(testingParams, tx, i,
				...
				mkGetKey(map[string]addressToKey{
					...
					scriptAddr.Address(): pkScript,
				}), sigScript, ucec.STEcdsaSecp256k1)
			if err != nil {
...
var (
	...
	thisAddressUnc, _ = ucutil.NewAddressPubKeyHash(
		ucutil.Hash160(thisPubKey.SerializeUncompressed()),
		testingParams, ucec.STEcdsaSecp256k1)
	uncompressedPkScript, _ = PayToAddrScript(thisAddressUnc)
	thisAddressCom, _       = ucutil.NewAddressPubKeyHash(
		ucutil.Hash160(thisPubKey.SerializeCompressed()),
		testingParams, ucec.STEcdsaSecp256k1)
