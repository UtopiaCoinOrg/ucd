...
func RawTxInSignatureAlt(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key chainec.PrivateKey, sigType ucec.SignatureType) ([]byte,
	error) {
...
func SignatureScriptAlt(tx *wire.MsgTx, idx int, subscript []byte,
	...
	sigType ucec.SignatureType) ([]byte,
	error) {
...
func p2pkSignatureScriptAlt(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, privKey chainec.PrivateKey, sigType ucec.SignatureType) ([]byte,
	error) {
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []ucutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
	// No need to add dummy in Utopia.
...
func handleStakeOutSign(tx *wire.MsgTx, idx int, subScript []byte,
	...
	addresses []ucutil.Address, class ScriptClass, subClass ScriptClass,
	nrequired int) ([]byte, ScriptClass, []ucutil.Address, int, error) {
...
func sign(chainParams ucutil.AddressParams, tx *wire.MsgTx, idx int,
	subScript []byte, hashType SigHashType, kdb KeyDB, sdb ScriptDB,
	sigType ucec.SignatureType) ([]byte,
	ScriptClass, []ucutil.Address, int, error) {
...
// does not accept a script version, the results are undefined for other script
// versions.
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []ucutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*ucutil.AddressSecpPubKey)
...
// does not accept a script version, the results are undefined for other script
// versions.
func mergeScripts(chainParams ucutil.AddressParams, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []ucutil.Address,
...
type KeyDB interface {
	GetKey(ucutil.Address) (chainec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(ucutil.Address) (chainec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address ucutil.Address) (chainec.PrivateKey, bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(ucutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(ucutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address ucutil.Address) ([]byte, error) {
	return sc(address)
}
...
// does not accept a script version, the results are undefined for other script
// versions.
func SignTxOutput(chainParams ucutil.AddressParams, tx *wire.MsgTx, idx int,
	pkScript []byte, hashType SigHashType, kdb KeyDB, sdb ScriptDB,
	previousScript []byte, sigType ucec.SignatureType) ([]byte, error) {
