...
// newAddressPubKey returns a new ucutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) ucutil.Address {
	pubkey, err := secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := ucutil.NewAddressSecpPubKeyCompressed(pubkey, mainNetParams)
	if err != nil {
		panic("invalid public key in test source")
	}
...
// newAddressPubKeyHash returns a new ucutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) ucutil.Address {
	addr, err := ucutil.NewAddressPubKeyHash(pkHash, mainNetParams,
...
// newAddressScriptHash returns a new ucutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) ucutil.Address {
	addr, err := ucutil.NewAddressScriptHashFromHash(scriptHash, mainNetParams)
	if err != nil {
...
// bogusAddress implements the ucutil.Address interface so the tests can ensure
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := ucutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
		"373273efcc54ce7d2a491bb4a0e84"), mainNetParams, ucec.STEcdsaSecp256k1)
	if err != nil {
	...
	p2shMain, _ := ucutil.NewAddressScriptHashFromHash(hexToBytes("e8c30"+
	...
	p2pkCompressedMain, err := ucutil.NewAddressSecpPubKey(hexToBytes("02192d7"+
	...
	p2pkCompressed2Main, err := ucutil.NewAddressSecpPubKey(hexToBytes("03b0b"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for Utopia this would
		// be uncompressed, but standard for Utopia is 33 byte
		...
		{(*ucutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ucutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*ucutil.AddressSecpPubKey)(nil), "", errUnsupportedAddress},
		{(*ucutil.AddressEdwardsPubKey)(nil), "", errUnsupportedAddress},
		{(*ucutil.AddressSecSchnorrPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := ucutil.NewAddressSecpPubKey(hexToBytes("02192d"+
	...
	tests := []struct {
		keys      []*ucutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*ucutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in Utopia.
			[]*ucutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*ucutil.AddressSecpPubKey),
			},
			1,
...
func TestGenerateSStxAddrPush(t *testing.T) {
	...
	for _, test := range tests {
		addr, err := ucutil.DecodeAddress(test.addrStr, test.net)
		if err != nil {
