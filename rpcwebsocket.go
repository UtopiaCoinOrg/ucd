...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *ucutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *ucutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *ucutil.Tx, isNew bool) {
...
func (f *wsClientFilter) addAddress(a ucutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) existsAddress(a ucutil.Address) bool {
	switch a := a.(type) {
	case *ucutil.AddressPubKeyHash:
...
func (m *wsNotificationManager) subscribedClients(tx *ucutil.Tx, clients map[chan struct{}]*wsClient) map[chan struct{}]struct{} {
...
func (m *wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient, block *ucutil.Block) {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *ucutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *ucutil.Tx) {
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			marshalledJSONVerbose, err = ucjson.MarshalCmd("1.0", nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *ucutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		...
		marshalled, err := ucjson.MarshalCmd("1.0", nil, n)
		if err != nil {
	...
	for {
		...
		if !batchedRequest {
			...
			if err != nil {
				...
				jsonErr := &ucjson.RPCError{
					Code:    ucjson.ErrRPCParse.Code,
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &ucjson.RPCError{
					Code:    ucjson.ErrRPCInvalidRequest.Code,
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &ucjson.RPCError{
						Code:    ucjson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = ucjson.MarshalResponse("2.0", nil, nil, jsonErr)
				if err != nil {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	if !ok {
		return nil, ucjson.ErrRPCInternal
	}
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *ucutil.Block) []string {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &ucjson.RPCError{
				Code:    ucjson.ErrRPCBlockNotFound,
