...
func shutdownListener() context.Context {
	...
	go func() {
		...
		case sig := <-interruptChannel:
			ucdLog.Infof("Received signal (%s).  Shutting down...", sig)

		case <-shutdownRequestChannel:
			ucdLog.Infof("Shutdown requested.  Shutting down...")
		}
		cancel()
		...
		for {
			select {
			case sig := <-interruptChannel:
				ucdLog.Infof("Received signal (%s).  Already "+
					"shutting down...", sig)

			case <-shutdownRequestChannel:
				ucdLog.Info("Shutdown requested.  Already " +
					"shutting down...")
			}
		}
	}()
