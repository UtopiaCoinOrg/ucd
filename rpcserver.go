...
var (
	...
	ErrRPCUnimplemented = &ucjson.RPCError{
		Code:    ucjson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &ucjson.RPCError{
		Code:    ucjson.ErrRPCNoWallet,
...
var rpcHandlersBeforeInit = map[types.Method]commandHandler{
	...
	"sendflashrawtransaction": handleSendFlashRawTransaction,
	"sendflashtxvote":         handleSendFlashTxVote,
...
// list of commands that we recognize, but for which ucd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of ucwallet.
...
// not needed.
func rpcInternalError(errStr, context string) *ucjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return ucjson.NewRPCError(ucjson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *ucjson.RPCError {
	return ucjson.NewRPCError(ucjson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *ucjson.RPCError {
	return ucjson.NewRPCError(ucjson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}

// rpcRuleError is a convenience function to convert a
// rule error to an RPC error with the appropriate code set.
func rpcRuleError(fmtStr string, args ...interface{}) *ucjson.RPCError {
	return ucjson.NewRPCError(ucjson.ErrRPCMisc,
		fmt.Sprintf(fmtStr, args...))
}
...
func rpcDuplicateTxError(fmtStr string, args ...interface{}) *ucjson.RPCError {
	return ucjson.NewRPCError(ucjson.ErrRPCDuplicateTx,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *ucjson.RPCError {
	return ucjson.NewRPCError(ucjson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *ucjson.RPCError {
	return ucjson.NewRPCError(ucjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *ucjson.RPCError {
	return ucjson.NewRPCError(ucjson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *ucjson.RPCError {
	return ucjson.NewRPCError(ucjson.ErrRPCMisc, message)
}
...
// These commands will be implemented in ucwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for _, input := range c.Inputs {
		...
		if input.Amount > 0 {
			amt, err := ucutil.NewAmount(input.Amount)
			if err != nil {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > ucutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, ucutil.MaxAmount)
		}
		...
		atomic, err := ucutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	for i, cout := range c.COuts {
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			ucutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > ucutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, ucutil.MaxAmount)
		}
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeAmt ucutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = ucutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > ucutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, ucutil.MaxAmount)
		}
...
func createVinList(mtx *wire.MsgTx) []types.Vin {
	...
	if blockchain.IsCoinBaseTx(mtx) {
		...
		vinEntry.AmountIn = ucutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, v := range mtx.TxOut {
		...
		if txType == stake.TxTypeSStx && (i%2 != 0) {
			...
			if err != nil {
				...
				addrs = []ucutil.Address{addr}
			}
			amt, err := stake.AmountFromSStxPkScrCommitment(v.PkScript)
			if err != nil {
				rpcsLog.Warnf("failed to decode tickehandleDecodeScriptt "+
		...
		vout.N = uint32(i)
		vout.Value = ucutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = ucjson.Float64(commitAmt.ToCoin())
		}
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := ucutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if c.Tickets != nil {
		...
		userEstFlt := ucutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &types.EstimateStakeDiffResult{
		Min:      ucutil.Amount(min).ToCoin(),
		Max:      ucutil.Amount(max).ToCoin(),
		Expected: ucutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	// Retrieve a list of persistent (added) peers from the Utopia server
	...
	for _, peer := range peers {
		...
		result.Connected = ucjson.Bool(peer.Connected())
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	sbitsFloat := float64(blockHeader.SBits) / ucutil.AtomsPerCoin
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blockHeaderReply := types.GetBlockHeaderVerboseResult{
		...
		PoolSize:      blockHeader.PoolSize,
		Bits:          strconv.FormatInt(int64(blockHeader.Bits), 16),
		SBits:         ucutil.Amount(blockHeader.SBits).ToCoin(),
...
func handleGetBlockTemplateRequest(s *rpcServer, request *types.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if bestHeight != 0 && !s.server.blockManager.IsCurrent() {
		return nil, &ucjson.RPCError{
			Code:    ucjson.ErrRPCClientInInitialDownload,
			Message: "Utopia is downloading blocks...",
		}
	}
...
// reasons.
// TODO Utopia pop in the new errors from blockchain cj
	...
	if c.Verbose != nil && *c.Verbose {
		...
		for i := range descs {
			...
			mpd := &types.GetRawMempoolVerboseResult{
				...
				Fee:              ucutil.Amount(desc.Fee).ToCoin(),
...
func handleGetTxLockpoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &types.GetTxLockpoolInfoResult{
		Info: info,
	}
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &ucjson.RPCError{
			Code:    ucjson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := ucutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := ucutil.Amount(nextSdiff)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &ucjson.RPCError{
				Code: ucjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &types.GetTxOutResult{
		...
		Confirmations: confirmations,
		Value:         ucutil.Amount(value).ToUnit(ucutil.AmountCoin),
...
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]types.VinPrevOut, error) {
	...
	if blockchain.IsCoinBaseTx(mtx) {
		...
		amountIn := ucutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &types.PrevOut{
				...
				Value:     ucutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr ucutil.Address, numToSkip, numRequested uint32) ([]*ucutil.Tx, uint32) {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if _, isFlash := txscript.IsFlashTx(msgtx); isFlash {
		if !s.server.txMemPool.IsFlashTxExistAndVoted(tx.Hash()) {
			return handleFlashTransaction(s, closeChan, serializedTx, allowHighFees)
		}
	}
...
func handleSendFlashRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
...
func handleFlashTransaction(s *rpcServer, closeChan <-chan struct{}, serializedTx []byte, allowHighFees bool) (interface{}, error) {

	bestShot := s.chain.BestSnapshot()
	if bestShot != nil {
		height := int64(bestShot.Height)
		if height < s.server.chainParams.StakeEnabledHeight {
			return nil, errors.New(fmt.Sprintf("Flash tx enable  height is too low, want %d, but get %d", s.server.chainParams.StakeEnabledHeight, height))
		}
	}
	...
	if !ok || lotteryHash == nil {
		return nil, fmt.Errorf("not a standard flashtx")
	}
	...
	s.server.AnnounceNewFlashTxs(flashTxs)
...
func handleSendFlashTxVote(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*types.SendFlashTxVoteCmd)
	...
	msgFlashTxVote := wire.NewMsgFlashTxVote()
	err = msgFlashTxVote.Deserialize(bytes.NewReader(serializedTx))
	if err != nil {
		return nil, rpcDeserializationError("Could not decode Flashtx vote: %v", err)
	}

	flashTxvote := ucutil.NewFlashTxVote(msgFlashTxVote)
	flashTxHash := msgFlashTxVote.FlashTxHash
	ticketHash := msgFlashTxVote.TicketHash
	...
	tickets, err := s.chain.LotteryFlashDataForTxAndBlock(&flashTxHash, lotteryHash)
	ticketExist := false
	...
	if !ticketExist {
		return nil, fmt.Errorf("flashtx ticket not exist ,flashvote %v: %v", flashTxvote.Hash(),
			err)
	}
	...
	//update lockpool
	err, reSendToMemPool := s.server.txMemPool.ProcessFlashTxVote(flashTxvote, &flashTxHash)
	if err != nil {
		return nil, err
	}
	if reSendToMemPool {
		s.ntfnMgr.NotifyFlashTx(tickets, flashTx, true)
	}
	...
	s.server.AnnounceNewFlashTxVotes(flashTxvotes)
	...
	s.server.AddRebroadcastInventory(iv, flashTxvote)
...
func min(s []ucutil.Amount) ucutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []ucutil.Amount) ucutil.Amount {
	max := ucutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
...
func mean(s []ucutil.Amount) ucutil.Amount {
	sum := ucutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / ucutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []ucutil.Amount) ucutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(ucutil.AmountSorter(s))
...
func stdDev(s []ucutil.Amount) ucutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := ucutil.NewAmount(math.Sqrt(v))
...
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *types.FeeInfoMempool {
	...
	ticketFees := make([]ucutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (ucutil.Amount(txD.Fee)) * 1000 /
				ucutil.Amount(txD.Tx.MsgTx().SerializeSize())
...
// properly set.
func calcFeePerKb(tx *ucutil.Tx) ucutil.Amount {
	var in ucutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += ucutil.Amount(txIn.ValueIn)
	}
	var out ucutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += ucutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / ucutil.Amount(tx.MsgTx().SerializeSize())
}
...
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*types.FeeInfoBlock, error) {
	...
	txFees := make([]ucutil.Amount, txNum)
	itr := 0
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return ucutil.Amount(vwap).ToCoin(), nil
}
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if _, ok := addr.(*ucutil.AddressPubKeyHash); !ok {
		return nil, &ucjson.RPCError{
			Code:    ucjson.ErrRPCType,
	...
	if wasCompressed {
		serializedPK = ucPK.SerializeCompressed()
	} else {
		serializedPK = ucPK.SerializeUncompressed()
	}
	address, err := ucutil.NewAddressSecpPubKey(serializedPK,
		activeNetParams.Params)
	if err != nil {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, ucjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *ucjson.Request) *parsedRPCCmd {
	...
	params, err := ucjson.ParseParams(types.Method(request.Method), request.Params)
	if err != nil {
		// When the error is because the method is not registered,
		// produce a method not found RPC error.
		if jerr, ok := err.(ucjson.Error); ok &&
			jerr.Code == ucjson.ErrUnregisteredMethod {
			parsedCmd.err = ucjson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
// type *ucjson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion string, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *ucjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*ucjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return ucjson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *ucjson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = ucjson.MarshalResponse("1.0", nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = ucjson.MarshalResponse("2.0", nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = ucjson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="ucd RPC"`)
