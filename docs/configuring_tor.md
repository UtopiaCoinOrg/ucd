...
ucd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, ucd supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Utopia
...
Configuring ucd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the ucd command
line or in the ucd configuration file.  Typically the Tor proxy address will be
...
By default, ucd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since ucd will act as the server.
...
```text
HiddenServiceDir /var/tor/ucd
HiddenServicePort 10508 127.0.0.1:10508
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring ucd as a Tor hidden service requires
three flags:
...
```bash
$ ./ucd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
ucd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the ucd command line or in the ucd
...
```bash
$ ./ucd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
ucd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./ucd --proxy=127.0.0.1:9050 --torisolation
```
