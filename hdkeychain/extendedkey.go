...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	parentFP := ucutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a ucec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*secp256k1.PublicKey, error) {
	return secp256k1.ParsePubKey(k.pubKeyBytes())
}

// ECPrivKey converts the extended key to a ucec private key and returns it.
...
// Address converts the extended key to a standard Utopia pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*ucutil.AddressPubKeyHash, error) {
	pkHash := ucutil.Hash160(k.pubKeyBytes())
	return ucutil.NewAddressPubKeyHash(pkHash, net, ucec.STEcdsaSecp256k1)
}
...
// passed Utopia network.
func (k *ExtendedKey) IsForNet(net *chaincfg.Params) bool {
	return bytes.Equal(k.privVer[:], net.HDPrivateKeyID[:]) ||
		bytes.Equal(k.pubVer[:], net.HDPublicKeyID[:])
}
...
func (k *ExtendedKey) SetNet(net *chaincfg.Params) {
	if k.isPrivate {
		copy(k.privVer[:], net.HDPrivateKeyID[:])
	} else {
		copy(k.pubVer[:], net.HDPublicKeyID[:])
	}
}
