...
type cpuminerConfig struct {
	...
	ProcessBlock func(*ucutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *ucutil.Block) bool {
	...
	minrLog.Infof("Block submitted via CPU miner accepted (hash %s, "+
		...
		ucutil.Amount(coinbaseTxGenerated))
	return true
}
...
out:
	for {
		...
		if m.solveBlock(template.Block, ticker, quit) {
			block := ucutil.NewBlock(template.Block)
			m.submitBlock(block)
