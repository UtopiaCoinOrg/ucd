...
var (
	...
	// ourselves to other Utopia peers.
	userAgentName = "ucd"
...
// broadcastMsg provides the ability to house a Utopia message to be broadcast
...
// server provides a Utopia server for handling communications to and from
// Utopia peers.
type server struct {
...
func (sp *serverPeer) OnBlock(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a ucutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := ucutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) AnnounceNewTransactions(txns []*ucutil.Tx) {
...
func (s *server) TransactionConfirmed(tx *ucutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices, ucdLookup, func(addrs []*wire.NetAddress) {
...
// AddedNodeInfo returns an array of ucjson.GetAddedNodeInfoResult structures
...
// newServer returns a new ucd server configured to listen on addr for the
// Utopia network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, dataDir string, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, ucdLookup)
	...
	feC := fees.EstimatorConfig{
		...
		MaxBucketFee: ucutil.Amount(fees.DefaultMaxBucketFeeMultiplier) * cfg.minRelayTxFee,
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The ucdLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := ucdLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
