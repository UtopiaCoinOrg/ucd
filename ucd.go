...
// winServiceMain is only invoked on Windows.  It detects when ucd is running
...
// ucdMain is the real main function for ucd.  It is necessary to work around
...
func ucdMain() error {
	...
	defer ucdLog.Info("Shutdown complete")

	// Show version and home dir at startup.
	ucdLog.Infof("Version %s (Go version %s %s/%s)", version.String(),
		runtime.Version(), runtime.GOOS, runtime.GOARCH)
	ucdLog.Infof("Home dir: %s", cfg.HomeDir)
	if cfg.NoFileLogging {
		ucdLog.Info("File logging disabled")
	}
	...
	if cfg.Profile != "" {
		go func() {
			...
			ucdLog.Infof("Creating profiling server "+
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			ucdLog.Errorf("Unable to create cpu profile: %v", err.Error())
			return err
		}
	...
	if cfg.MemProfile != "" {
		...
		if err != nil {
			ucdLog.Errorf("Unable to create mem profile: %v", err)
			return err
		}
		defer f.Close()
	...
	defer func() {
		...
		ucdLog.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		ucdLog.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventP2PServer)
		ucdLog.Infof("Gracefully shutting down the server...")
		server.Stop()
