...
func containsTxIns(txs []*ucutil.Tx, tx *ucutil.Tx) bool {
...
// can not be found.
func txIndexFromTxList(hash chainhash.Hash, list []*ucutil.Tx) int {
	for i, tx := range list {
		h := tx.Hash()
		if hash == *h {
			return i
...
func UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int64, extraNonce uint64) error {
	...
	// TODO(davec): A ucutil.Block should use saved in the state to avoid
	// recalculating all of the other transaction hashes.
	// block.Transactions[0].InvalidateCache()

	// Recalculate the merkle root with the updated extra nonce.
	block := ucutil.NewBlockDeepCopyCoinbase(msgBlock)
...
func createCoinbaseTx(subsidyCache *blockchain.SubsidyCache, coinbaseScript []byte, opReturnPkScript []byte, nextBlockHeight int64, addr ucutil.Address, voters uint16, params *chaincfg.Params) (*ucutil.Tx, error) {
	tx := wire.NewMsgTx()
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *ucutil.Tx, height int64) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func logSkippedDeps(tx *ucutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
func maybeInsertStakeTx(bm *blockManager, stx *ucutil.Tx, treeValid bool) bool {
	missingInput := false
...
func deepCopyBlockTemplate(blockTemplate *BlockTemplate) *BlockTemplate {
	...
	coinbaseCopy :=
		ucutil.NewTxDeep(blockTemplate.Block.Transactions[0])
...
func handleTooFewVoters(subsidyCache *blockchain.SubsidyCache, nextHeight int64, miningAddress ucutil.Address, bm *blockManager) (*BlockTemplate, error) {
	...
	if nextHeight >= stakeValidationHeight {
		if bm.AggressiveMining {
			if curTemplate != nil {
				...
				block := ucutil.NewBlockDeepCopyCoinbase(cptCopy.Block)
			...
			btBlockTemp := ucutil.NewBlockDeepCopyCoinbase(btMsgBlock)
			merkles :=
			...
			// Make sure the block validates.
			btBlock := ucutil.NewBlockDeepCopyCoinbase(btMsgBlock)
...
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress ucutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*ucutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		// Consider caching this in the mempool in the future. - Utopia
	...
	blockTxnsStake := make([]*ucutil.Tx, 0, len(blockTxns))
	...
	if nextBlockHeight >= stakeValidationHeight {
		for _, tx := range blockTxns {
			...
			if stake.IsSSGen(msgTx) {
				txCopy := ucutil.NewTxDeepTxIns(msgTx)
	...
	if nextBlockHeight < stakeValidationHeight {
		...
		for _, vb := range voteBitsVoters {
			if ucutil.IsFlagSet16(vb, ucutil.BlockValid) {
				voteYea++
			}
			totalVotes++
		}
		...
		if votebits == uint16(0x0000) {
			...
			// the previous block.
			// UTOPIA WARNING: The ideal behaviour should also be that we re-add
			...
			tempBlockTxns := make([]*ucutil.Tx, 0, len(sourceTxns))
			for _, tx := range blockTxns {
				if tx.Tree() == wire.TxTreeRegular {
					...
					if isValid {
						txCopy := ucutil.NewTxDeepTxIns(tx.MsgTx())
	...
	// Utopia: We need to move this downwards because of the requirements
	...
	blockTxnsRegular := make([]*ucutil.Tx, 0, len(blockTxns)+1)
	...
	minrLog.Debugf("Created new block template (%d transactions, %d "+
		...
		ucutil.Amount(msgBlock.Header.SBits).ToCoin())
...
//  - rtReorgStarted:      nil
//  - rtReorgDone:         nil
//  - rtBlockAccepted:     *ucutil.Block
//  - rtBlockConnected:    *ucutil.Block
//  - rtBlockDisconnected: *ucutil.Block
//  - rtVote:              *ucutil.Tx
...
func newBgBlkTmplGenerator(tg *BlkTmplGenerator, addrs []ucutil.Address, allowUnsynced bool) *BgBlkTmplGenerator {
...
func (g *BgBlkTmplGenerator) handleBlockConnected(ctx context.Context, state *regenHandlerState, block *ucutil.Block, chainTip *blockchain.BestState) {
...
//
// This function is only intended for use by the regen handler goroutine.
func (g *BgBlkTmplGenerator) handleBlockDisconnected(ctx context.Context, state *regenHandlerState, block *ucutil.Block, chainTip *blockchain.BestState) {
	// Clear all vote tracking when the current chain tip changes.
	state.awaitingMinVotesHash = nil
	state.clearSideChainTracking()
...
//
// This function is only intended for use by the regen handler goroutine.
func (g *BgBlkTmplGenerator) handleBlockAccepted(ctx context.Context, state *regenHandlerState, block *ucutil.Block, chainTip *blockchain.BestState) {
...
//
// This function is only intended for use by the regen handler goroutine.
func (g *BgBlkTmplGenerator) handleVote(ctx context.Context, state *regenHandlerState, voteTx *ucutil.Tx, chainTip *blockchain.BestState) {
...
func (g *BgBlkTmplGenerator) handleRegenEvent(ctx context.Context, state *regenHandlerState, event regenEvent) {
	...
	case rtBlockConnected:
		block := event.value.(*ucutil.Block)
...
//
// This function is safe for concurrent access.
func (g *BgBlkTmplGenerator) BlockAccepted(block *ucutil.Block) {
...
//
// This function is safe for concurrent access.
func (g *BgBlkTmplGenerator) BlockConnected(block *ucutil.Block) {
...
//
// This function is safe for concurrent access.
func (g *BgBlkTmplGenerator) BlockDisconnected(block *ucutil.Block) {
...
//
// This function is safe for concurrent access.
func (g *BgBlkTmplGenerator) VoteReceived(tx *ucutil.Tx) {
